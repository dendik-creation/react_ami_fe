...
const MasterIso: React.FC = () => {
  ...
  //   Edit
  const [isoSelected, setSelectedIso] = useState<any>();
  ...
  useEffect(() => {
    api.isoAll(setLoading, 1, setPaginating).then((res) => setIsoList(res));
  }, []);
  const handlePaginate = (target_page: number) => {
    setPaginating(true);
    api
      .isoAll(setLoading, target_page, setPaginating)
  ...
  const handleEdit = (id: number, index: number) => {
    setSelectedIso(isoList?.iso?.data[index]);
  ...
  useEffect(() => {
    setTimeout(() => {
      if (search.length > 3) {
        setPaginating(true);
        api
          .isoAll(setLoading, 1, setPaginating, search)
          .then((res) => setIsoList(res));
  ...
  useEffect(() => {
    if (successDelete || successUpdate || successCreate) {
      setLoading(true);
      api
        .isoAll(setLoading, 1, setPaginating)
        .then((res) => {
          setIsoList(res);
          setShowModal(false);
    ...
    <>
      <DefaultLayout>
        <ConfirmDelete
          ...
          target="DEL-ISO"
          message="Menghapus ISO menyebabkan terhapusnya standar yang digunakan setiap proses audit"
        ...
        <div className="mb-12">
          ...
          >
            <Breadcrumb
              pageName="Data ISO"
              description={
                'Standar ISO yang digunakan pada sistem ditampilkan disini'
              }
            />
            <button
            ...
            >
              ...
              <span className="font-medium">ISO Baru</span>
            </button>
          </Transition>
          ...
          >
            ...
            <input
              ...
              placeholder="Cari Berdasarkan Kode ISO"
                ...
                <table className="w-full table-auto mb-4">
                  ...
                  <tbody>
                    {isPaginating
                      ...
                      : isoList?.iso?.data?.map((item: any, index: number) => (
                          <tr key={index}>
                ...
                <div className="m-2">
                  <nav className="flex justify-between items-center">
                    <ul className="flex flex-wrap items-center gap-3">
                      <li>
                        <button
                          disabled={isoList?.meta_paginate?.current_page == 1}
                          onClick={() =>
                            handlePaginate(
                              isoList?.meta_paginate?.current_page - 1,
                            )
                          }
                      ...
                      <li>
                        <button
                          disabled={
                            isoList?.meta_paginate?.current_page ==
                            isoList?.meta_paginate?.last_page
                          }
                          onClick={() =>
                            handlePaginate(
                              isoList?.meta_paginate?.current_page + 1,
                    ...
                    <div className="me-3 flex gap-3 items-center">
                      ...
                      <div className="mt-1">Total {isoList?.iso_count} ISO</div>
                    </div>
                  </nav>
                </div>
              </div>
