...
import { FiSave, FiTrash, FiUserPlus, FiUserX } from 'react-icons/fi';
...
const MasterGrupAuditorEdit: React.FC = () => {
  const [grupAuditor, setGrupAuditor] = useState<GrupAuditorForm>();
  const [auditorListSelect, setAuditorSelect] = useState<
    ListSelect[] | undefined
  >();
  const [auditorRemoved, setAuditorRemoved] = useState<any[]>([]);
  ...
  useEffect(() => {
    if (id) {
      api
        .getAuditorListSelect(id)
        .then((res) => setAuditorSelect(res))
        .catch((err) => console.log(err));
      api
        .getGrupAuditorDetail(setLoading, id)
        .then((res) => {
          helper
            .responseConvertState(res)
            .then((lastRes: any) => setGrupAuditor(lastRes))
  ...
  useEffect(() => {
    ...
    console.log(auditorRemoved);
  }, [sendForm, grupAuditor, auditorRemoved]);
  ...
  const handleRemoveAnggota = (index: number) => {
    const auditor_listed = [...grupAuditor?.auditor_list];
    if (auditor_listed[index]?.status == 'then') {
      setAuditorRemoved([...auditorRemoved, grupAuditor?.auditor_list[index]]);
    }
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    helper
      .readyFormSendUpdate(grupAuditor, auditorRemoved)
  ...
  return (
    <DefaultLayout>
      <ConfirmSubmit
        ...
        target="GRUP-AUDITOR"
        method="put"
        message="Apakah anda yakin untuk memperbarui data grup auditor?"
      />
      ...
      <div className="mb-12">
        ...
        >
          <Breadcrumb
            pageName="Edit Data Grup Auditor"
            description={
              'Anda dapat menambah atau mengurangi auditor yang dikelompokkan dalam satu grup'
            }
          />
        </Transition>
            ...
            <div className="flex flex-col gap-5.5 p-6.5">
              ...
              <div className="grid grid-cols-1">
                <div>
                  ...
                  {grupAuditor?.auditor_list?.length > 0 ? (
                    ...
                    <div className="flex items-center gap-2">
                      <FiUserX className="text-slate-500 text-xl" />
                      <span className="text-slate-700">
                        Grup Tidak Memiliki Anggota
                      </span>
                    </div>
                  )}
                </div>
...
export default MasterGrupAuditorEdit;
