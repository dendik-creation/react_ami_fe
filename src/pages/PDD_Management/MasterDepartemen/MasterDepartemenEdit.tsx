...
const MasterDepartemenEdit: React.FC = () => {
  ...
  useEffect(() => {
    apiDept
      .departemenDetail(setLoading, id)
      .then((res) => setDepartemen(res))
  ...
  useEffect(() => {
    ...
    console.log(subDeptRemoved);
  }, [sendForm, departemen, subDeptRemoved]);
  ...
  const handleRemoveSubDept = (index: number) => {
    const subDepts = [...departemen?.sub_departemen];
    if (subDepts[index]?.status == 'then') {
      setRemoved([...subDeptRemoved, departemen?.sub_departemen[index]?.id]);
    }
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    helper
      .readyFormSendUpdate(departemen, subDeptRemoved)
  ...
  return (
    <DefaultLayout>
      <ConfirmSubmit
        ...
        target="DEPT"
        method="put"
        message="Apakah anda yakin untuk memperbarui data departemen beserta subnya ?"
      />
      ...
      <div className="mb-12">
        ...
        >
          <Breadcrumb
            pageName="Edit Data Departemen"
            description={'Anda dapat mengubah informasi terkait departemen'}
          />
        </Transition>
...
export default MasterDepartemenEdit;
