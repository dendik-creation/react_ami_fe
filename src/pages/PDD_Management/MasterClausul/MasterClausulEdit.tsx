...
import { api, helper } from '../../../api/master_clausul';
...
const MasterClausulEdit: React.FC = () => {
  const [isoList, setIsoList] = useState<any[]>();

  const [judul_clausul, setClausul] = useState<JudulClausul | undefined>();
  const [loading, setLoading] = useState<boolean>(true);
  const { id } = useParams();
  const [sendForm, setForm] = useState<any>(null);
  const [removedClausul, setRemovedClausul] = useState<{
    clausul: Clausul[];
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        helper
          .setStatusClausulList(res)
          .then((finalRes: any) => setClausul(finalRes))
          .catch((err) => console.log(err));
        setLoading(false);
      })
      .catch((err) => console.log(err));
  ...
  const handleSubClausul = {
    ...
    async removeSubClausul(
      ...
      if (status == 'then') {
        setRemovedClausul((prev: any) => {
          const clausulsArray = [...judul_clausul.clausul];
          ...
          return {
            ...
            sub_clausul: [
              ...prev.sub_clausul,
              subClausulArray[sub_clausul_index],
            ],
          };
        });
      }
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setForm({
      updatable: judul_clausul,
      removed: removedClausul,
    });
  ...
  return (
    <DefaultLayout>
      <ConfirmSubmit
        ...
        target="CLAUSUL"
        method="put"
        message="Apakah anda yakin akan memperbarui data standar clausul?"
      />
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row w-full px-6.5 py-4.5 gap-5">
              ...
              <div className="flex md:w-1/2 w-full flex-col gap-2">
                <span className="">Standar ISO</span>
                <ReactSelect
                  required
                  options={isoList}
                  name="iso_id"
                  id="iso_id"
                  onChange={(e) => handleSelect(e, 'iso_id')}
                  value={judul_clausul?.iso_id}
                  placeholder="Pilih ISO"
                  noOptionsMessage={() => 'ISO Tidak Ada'}
...
export default MasterClausulEdit;
